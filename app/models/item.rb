...
class Item < ApplicationRecord
  ...
  def create_ingredients_from_names(names)
    names.map do |name|
      ...
      Ingredient.new(name.strip, nested: nested_ingredients, and_or: and_or, description: @description) #, nested: nested_ingedients)
    end
  end
  ...
  def parse_nested_ingredients(ingredient_string)
    ...
    return create_ingredients_from_names(ingredient_names) if ingredient_names.count > 1

    @description = ingredient_names.last # TODO: refactor the description into its own method
    nil
  end
end
